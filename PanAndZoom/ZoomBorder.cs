...
public partial class ZoomBorder : Border
{
    ...
    public ZoomBorder()
    {
        ...
        this.GetObservable(EnableGesturesProperty)
    ...
    public static Matrix CalculateMatrix(double panelWidth, double panelHeight, double elementWidth,
        double elementHeight, StretchMode mode)
    {
    ...
    private void UpdateGestureRecognizers()
    {
        if (EnableGestures && !_gestureRecognizersAdded)
        {
            ...
            if (_pinchGestureRecognizer != null) GestureRecognizers.Add(_pinchGestureRecognizer);

            // Add scroll gesture recognizer only if not disabled by ScrollViewer parent
            if (_scrollGestureRecognizer != null) GestureRecognizers.Add(_scrollGestureRecognizer);
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        if (_element == null || !_element.IsMeasureValid) return size;
    ...
    private void Border_ScrollGesture(object? sender, ScrollGestureEventArgs e)
    {
        ...
        var gestureArgs = new GestureEventArgs(
            ...
            0,
            0,
            Sqrt(e.Delta.X * e.Delta.X + e.Delta.Y * e.Delta.Y),
            _matrix,
            previousMatrix
        );
        RaiseGestureStarted(gestureArgs);
    ...
    {
        if (e.Property == BoundsProperty) InvalidateScrollable();
    }
    ...
    private void ChildChanged(Control? element)
    {
        ...
        if (element != null && element != _element) AttachElement(element);
    }
    ...
    {
        if (_element == null || _captured) return;
    ...
    private void PanningFinished()
    {
        ...
        if (_element == null || !_captured || !_isPanning) return;
    ...
    private void Moved(PointerEventArgs e)
    {
        ...
        if (_element == null || !_captured || !_isPanning || !IsPanButtonPressed(e)) return;
    ...
    /// </summary>
    /// <param name="skipTransitions">
    ///     The flag indicating whether transitions on the child element should be temporarily
    ///     disabled.
    /// </param>
    ...
    private void InvalidateElement(bool skipTransitions)
    {
        ...
        if (skipTransitions && backupTransitions != null)
        {
            ...
            if (anim != null) anim.Transitions = backupTransitions;
        }
    ...
    public void ZoomTo(double ratio, double x, double y, bool skipTransitions = false)
    {
        ...
        if ((ZoomX >= MaxZoomX && ZoomY >= MaxZoomY && ratio > 1) ||
